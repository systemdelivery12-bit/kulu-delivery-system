...
const requireApprovedDriver = async (req, res, next) => {
  ...
  if (driver.rows.length === 0) {
    return res.status(403).json({ success: false, error: { code: 'NOT_APPROVED' } });
  }
  next();
};
...
router.get('/wallet', driverCtrl.getWallet);
router.get('/coin-packages', driverCtrl.getAvailablePackages);
router.post('/coin-packages/:id/purchase', driverCtrl.purchasePackage);
